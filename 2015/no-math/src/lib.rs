...
impl PresentBox {
  ...
  fn box_area(&self) -> u64 {
    self.sides_areas().into_iter().fold(0, |acc, area| acc + (2 * area))
  }

  fn slack_area(&self) -> u64 {
    self.sides_areas().into_iter().min().expect("Couldn't find minimum area")
  }
  ...
  pub fn required_ribbon(&self) -> u64 {
    self.side_perimeters().into_iter().min().expect("Could not find minimum perimeter") + self.volume()
  }
}
