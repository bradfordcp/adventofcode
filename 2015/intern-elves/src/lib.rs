...
fn vowel_count(subject: &str) -> usize {
  subject.chars().filter(|c| VOWELS.contains(c.to_string().as_str())).count()
}
...
fn has_forbidden_strings(subject: &str) -> bool {
  ...
  forbidden.into_iter().map(|pat| subject.contains(pat)).fold(false, |acc, cond| acc || cond)
}
...
fn has_repeating_pair(subject: &str) -> bool {
  ...
  // Generate all pairs
  let mut iter = subject.chars();
  let mut last = iter.next().expect("Could not obtain character from subject");
  let mut pairs = vec![];
...
pub fn is_nice(protocol: NiceProtocol, subject: &str) -> bool {
  match protocol {
    NiceProtocol::V1 => has_at_least_3_vowels(subject) && has_repeating_characters(subject, 1) && !has_forbidden_strings(subject),
